...
public class ExShowSeedSetting extends L2GameServerPacket
{
	...
	public void writeImpl()
	{
		...
		for(int i = 0; i < _count; i++)
		{
			...
			writeQ((int) _seedData[i * 12 + 8]); // today sales
			writeQ((int) _seedData[i * 12 + 9]); // today price
			writeQ((int) _seedData[i * 12 + 10]); // next sales
			writeQ((int) _seedData[i * 12 + 11]); // next price
		}
	}
	...
	public void writeImplIT()
	{
		...
		for(int i = 0; i < _count; i++)
		{
			...
			writeD((int) _seedData[i * 12 + 8]); // today sales
			writeD((int) _seedData[i * 12 + 9]); // today price
			writeD((int) _seedData[i * 12 + 10]); // next sales
			writeD((int) _seedData[i * 12 + 11]); // next price
		}
	}
}