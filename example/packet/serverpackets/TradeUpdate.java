...
public class TradeUpdate extends AbstractItemListPacket
{
	private int _type;
	private L2ItemInstance temp;
	private int _amount;

	public TradeUpdate(int type, L2ItemInstance x, int amount)
	{
		_type = type;
		temp = x;
		_amount = amount;
	...
	protected final void writeImpl()
	{
		...
		else if(_type == 2)
		{
			writeD(1); // Count
			writeD(1);
			writeH(_amount > 0 && temp.getItem().isStackable() ? 3 : 2);
